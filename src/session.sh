...
# Version 0.9.0, released 04-12-2011.
#
...
[ "$HOME" ] || { reportError "Variable HOME not set" ; exit 1 ; }
# FIXME: If HOME is not set, perhaps use /var/lib/session?
usrcfd="$HOME/.session"
...
if [ "$platform" = "linux" -o "$platform" = "macosx" ] ; then
    ...
    if [ "$sshkey" -a "$agent" ] ; then
        ...
        if [ "$SSH_AUTH_SOCK" ]; then
            ...
            reportDebug "I'm using lsof to do this and $privesc to elevate privileges"
            sshagentlsof="$($privesc lsof /tmp/ssh-*/agent.* 2> /dev/null | grep ssh-agent | tail -n1)"
...
printState() {
    ...
    case "$1" in
      long)
        ...
        if [ "$type" != "group" ] ; then
            echo -n "groups="
            local groups="$(debug=0 verbose=1 ; listHelper group | grep "$name" | cut -d " " -f 2 | cut -d "(" -f 1 | tr '\n' ',' | sed  "s|,$||")"
            if [ "$groups" ]; then echo "'$groups'"; else echo "'none'"; fi
        fi

        if [ "$type" = "host" -o "$type" = "guest" ] ; then
            echo -n "services="
            local services="$(debug=0 verbose=1 ; listHelper service | grep "$name" | cut -d " " -f 2 | cut -d "(" -f 1 | tr '\n' ',' | sed  "s|,$||")"
            if [ "$services" ]; then echo "'$services'"; else echo "'none'"; fi
        fi

        if [ "$type" = "host" -a "$vrmt" != "none" ] ; then
            echo -n "guests="
            local guests="$(debug=0 verbose=1 ; listHelper guest | grep "$name" | cut -d " " -f 2 | cut -d "(" -f 1 | tr '\n' ',' | sed  "s|,$||")"
...
addConf() {
    ...
    if [ "$entryFound" ] ; then
        reportError "An entry with name $name is already present in $usrcff: '$entryFound'"
        return 1
    fi
...
plinkTellCommandWriter() {
    ...
    echo "plink -x $sshopts -l \"$user\" $addr \"$command\""
}
...
# mapEntry(<arg> <space_separated_list_of_functions> [<return_format>])
#
# Execute the members of the <comma_separated_list_of_functions> sequentially passing <arg> to each one.
#
mapEntry() {
    reportDebugFuncEntry "$*"
    ...
    if [ "$type" = "group" ] ; then
        ...
        let ndx=1
        local IFS=","
        for grpentity in $grplist ; do
            parseEntry "$grpentity"
            if [ "$mode" = "serial" ] ; then
                local IFS=","
                for function in $functions ; do
                    if [ "$function" = "printState" ] ; then
                        doFunc "$function" "$returnformat"
                        let retval=$((retval|$?))
                    else
                        doFunc "$function" "$grpentity"
                        let retval=$((retval|$?))
                    fi
                done
                ...
                curstate="unknown"
                while [ "$curstate" != "$targetstate" ] ; do
                    curstate="$(IFS=" " ; checkState ; echo "$state")"
                    if [ "$curstate" != "$targetstate" ] ; then
                        echo "$name: waiting for $grpentity to go $targetstate (currently $curstate)"
                        sleep 1
                    else
                        echo "$name: $grpentity is $curstate, continuing..."
                    fi
                done
            else
                reportBug "Unknown mode $mode passed"
                exit 1
            fi
            let ndx+=1
        done
        ...
        # It is either a host, guest or service.
        # Proceed.
        if [[ "$functions" =~ stop ]] ; then
            targetstate="off"
        else
        ...
        if [ "$mode" = "stateful" ] ; then
            ...
            while [ "$curstate" != "$targetstate" ] ; do
                curstate="$(IFS=" " ; checkState ; echo "$state")"
                if [ "$curstate" != "$targetstate" ] ; then
                    echo "$name: waiting to go $targetstate (currently $curstate)"
                    sleep 1
                else
                    state="$curstate"
                    printState short
    ...
    # Do certain access related things when mapEntry loop exits.
    if [[ "$functions" =~ "accessEntity" ]] ; then
        # Reconnect to the generated screen session before exiting mapEntry.
        if [ "$terminal" = "screen" -a "$TERM" != "screen" ] ; then
            sleep 1
            screen -r
        # Set the title of the local (first) window before exiting mapEntry.
...
case "$main" in
  addconf)
    ...
    shift 2
    mandatories="type" optionals="$optionals_all $optionals_hosts $optionals_guests $optionals_services $optionals_groups" silence="true" parseParameters "$@"

    if [ "$type" = "host" -a "$vrmt" = "none" ] ; then
        mandatories="type osmt acmt exmt user admin addr" optionals="vrmt" parseParameters "$@"
        ...
        mandatories="type osmt acmt exmt user admin addr vrmt vmhome vmdata" parseParameters "$@"
        tokenValidator "type,name,osmt,acmt,exmt,user,admin,addr,vrmt" || exit 1
        addConf "$type $name($osmt,$acmt,$exmt,$user,$admin,$addr,$vrmt)" || exit 1
        ...
        mandatories="type osmt acmt exmt user admin addr host" parseParameters "$@"
        ...
        mandatories="type svmt addr port host" optionals="acmt exmt user" parseParameters "$@"
        ...
        mandatories="type svmt host svstatus svstart svstop" optionals="acmt exmt user addr port" parseParameters "$@"
        tokenValidator "type,name,svmt,acmt,exmt,user,addr,port,host" || exit 1
        addConf "$type $name($svmt,$acmt,$exmt,$user,$addr,$port,$host)" || exit 1
        ...
        mandatories="type members" optionals="mode" parseParameters "$@"
  ...
  modconf)
    ...
    if [ "$type" = "host" ] ; then
        ...
        if [ "$oldtype" = "guest" ] ; then
            ...
            mandatories="type vrmt $mandatory_extras" optionals="name osmt acmt exmt user admin addr $optional_extras" parseParameters "$@"
        elif [ "$oldtype" = "service" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type osmt vrmt admin $mandatory_extras" optionals="name acmt exmt user addr $optional_extras" parseParameters "$@"
        elif [ "$oldtype" = "group" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type osmt acmt exmt user admin addr vrmt $mandatory_extras" optionals="name $optional_extras" parseParameters "$@"
        else
            reportDebug "Setting default mandatories and optionals for $type"
            mandatories="type $mandatory_extras" optionals="name osmt acmt exmt user admin addr vrmt $optional_extras" parseParameters "$@"
        fi
    ...
    # Handle a GUEST modification.
        ...
        if [ "$oldtype" = "host" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type host" optionals="name osmt acmt exmt user admin addr" parseParameters "$@"
        elif [ "$oldtype" = "group" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type osmt acmt exmt user admin addr host" optionals="name" parseParameters "$@"
        elif [ "$oldtype" = "service" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type osmt admin host" optionals="name acmt exmt user addr" parseParameters "$@"
        else
            reportDebug "Setting default mandatories and optionals for $type"
            mandatories="type" optionals="name osmt acmt exmt user admin addr host" parseParameters "$@"
        fi
    ...
    # Handle a SERVICE modification.
        ...
        if [ "$oldtype" = "host" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type svmt port host $mandatory_extras" optionals="name acmt exmt user addr $optional_extras" parseParameters "$@"
        elif [ "$oldtype" = "guest" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type svmt port $mandatory_extras" optionals="name acmt exmt user addr host $optional_extras" parseParameters "$@"
        elif [ "$oldtype" = "group" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type svmt acmt exmt user addr port host $mandatory_extras" optionals="name $optional_extras" parseParameters "$@"
        else
            reportDebug "Setting default mandatories and optionals for $type"
            mandatories="type $mandatory_extras" optionals="name svmt acmt exmt user addr port host $optional_extras" parseParameters "$@"
        fi
    ...
    # Handle a GROUP modification.
        ...
        if [ "$oldtype" != "group" ] ; then
            reportDebug "Rewriting mandatories and optionals to reflect a $type since old type was $oldtype"
            mandatories="type members" optionals="name mode" parseParameters "$@"
        else
            reportDebug "Setting default mandatories and optionals for $type"
            mandatories="type" optionals="name members mode" parseParameters "$@"
        fi
  ...
  check|state|status)
    ...
    shift 2
    optionals="mode" silence="true" parseParameters "$@"

    # Look up entry for given input.
    mapEntry "$name" "checkState,printState"
    exit $?
    ;;
  detail)
    [ "$2" ] || { printUsageText ; exit 1 ; }
  ...
  start)
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,startEntity,printState"
    exit $?
    ;;
  stop)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,stopEntity,printState"
    exit $?
    ;;
  restart|reboot)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,restartEntity,printState"
    exit $?
    ;;
  create)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    shift 2
    mandatories="numvcpu memsize dsksize guestos" optionals="desc" parseParameters "$@"
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,createEntity,printState"
    exit $?
    ;;
  destroy)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,destroyEntity,printState"
    exit $?
    ;;
  access)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    shift 2
    optionals="user admin service mode" silence="true" parseParameters "$@"
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,accessEntity"
    exit $?
    ;;
  tell)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    shift 2
    mandatories="command" optionals="user admin service mode" silence="true" parseParameters "$@"
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,runAs"
    exit $?
    ;;
  send)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    shift 2
    mandatories="source target" optionals="user admin service mode" silence="true" parseParameters "$@"
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,sendAs"
    exit $?
    ;;
  key)
    [ "$2" ] || { printUsageText ; exit 1 ; }
    ...
    # Look up entry for given input.
    mapEntry "$name" "checkState,credAs"
    exit $?
    ;;
  reinit)
  ...
  list)
    ...
    name="$2"
    shift 2
    optionals="default verbose" silence="true" parseParameters "$@"
