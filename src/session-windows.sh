...
# Version 0.7.8, released 13-01-2011.
#
...
known_osses="embedded-like unix-like windows-like aix5 aix6 arch2kx debian4 debian5 dfbsd2 esxi4 fbsd7 fbsd8 fedora13 fedora14 hpux1123 hpux1131 macosx6 maemo5 nbsd4 nbsd5 obsd4 osuse11 rhel4 rhel5 sles11 sol10 ubuntu8 ubuntu10 win2k3 win2k8 win7 winxp"
...
sshSendKey(){
    ...
    if [ ! -z $sshkey ]; then
        ...
        pscp $pkopts -q "`cygpath -m $sshkey`" $sender@$addr:/tmp/pubkey
        if [ "$?" == "0" ]; then
...
case "$1" in 
    ...
    if [ "$type" = "host" ]; then
        if [ "$oldtype" = "guest" ]; then
            ...
            mandatory="type os acmt exmt user admin addr vrmt"
            optional="name"
            parseParameters $@
        else
            mandatory="type"
