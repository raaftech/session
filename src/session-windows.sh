...
# Version 0.7.10, released 25-01-2011.
#
...
tools_cygwin="awk cut grep sed uuidgen"
tools_native="nslookup mstsc robocopy"
tools_extras="cryptrdp5 nmap putty pageant plink pscp psexec"
tools="$tools_cygwin $tools_native $tools_extras"
...
if [ -e "$HOME/.ssh/id_dsa.ppk" ]; then
    pkfile="`cygpath -m $HOME/.ssh/id_dsa.ppk`"
    pkopts="-i $pkfile"
elif [ -e "$HOME/.ssh/id_rsa.ppk" ]; then
    pkfile="`cygpath -m $HOME/.ssh/id_rsa.ppk`"
    pkopts="-i $pkfile"
fi

# Setup pageant to handle caching of private key credentials.
if [ "$agent" != 0 ]; then agent=true; fi
if [ "$pkfile" -a "$agent" = true ]; then
    running=`tasklist | grep -i pageant`
    if [ ! "$running" ]; then
        echo "you have a private key; loading into pageant"
        pageant "$pkfile" 2>/dev/null &
        sleep 8
    fi
fi

known_osses="embedded-like unix-like windows-like aix5 aix6 arch2kx debian4 debian5 dfbsd2 esxi4 fbsd7 fbsd8 fedora13 fedora14 hpux1123 hpux1131 macosx6 maemo5 nbsd4 nbsd5 obsd4 osuse11 rhel4 rhel5 rhel6 sles11 sol10 ubuntu8 ubuntu10 win2k3 win2k8 win7 winxp"
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        nmap -n -T5 -PE -oG `cygpath -m /tmp/session.discover.out` -sP $range > /dev/null 2>&1 < /dev/null
...
noneVirtHandler(){
    ...
    case "$1" in
        state)
        nmap -n -T5 -PE -sP $addr 2> /dev/null | grep -q "is up" && echo reachable || echo unreachable
        ;;
        start)
...
sshSendKey(){
    ...
    if [ ! -z $sshkey ]; then
        ...
        pscp $pkopts -scp -q "`cygpath -m $sshkey`" $sender@$addr:/tmp/pubkey
        if [ "$?" == "0" ]; then
        ...
        plink $pkopts $sender@$addr 'mkdir -p $HOME/.ssh; touch $HOME/.ssh/authorized_keys; cat $HOME/.ssh/authorized_keys /tmp/pubkey | sort | uniq > /tmp/authorized_keys; mv /tmp/authorized_keys $HOME/.ssh/authorized_keys; rm /tmp/pubkey; chmod 755 $HOME; chmod 755 $HOME/.ssh; chmod 600 $HOME/.ssh/authorized_keys'
        if [ "$?" == "0" ]; then
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        runasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "elevation of local privileges when using psexec not implemented yet"
        elif [ "$execute" = open ]; then
            echo "@echo off"                                         > /tmp/session.command.$name.bat
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "@echo off"                                        > /tmp/session.command.$name.bat
            echo "plink $pkopts $user@$addr \"$command\""          >> /tmp/session.command.$name.bat
            cmd.exe /c `cygpath -m /tmp/session.command.$name.bat`
            rm /tmp/session.command.$name.bat
        fi
        ;;
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "@echo off"                                        > /tmp/session.command.$name.bat
            echo "plink $pkopts $admin@$addr \"$command\""         >> /tmp/session.command.$name.bat
            cmd.exe /c `cygpath -m /tmp/session.command.$name.bat`
            rm /tmp/session.command.$name.bat
        fi
        ;;
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            pscp $pkopts -scp -p -q -r "`cygpath -m $source`" $user@$addr:"$target"
        fi
        ;;
        sendasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            pscp $pkopts -scp -p -q -r "`cygpath -m $source`" $admin@$addr:"$target"
        fi
        ;;
        *)
    esac
...
case "$1" in 
    ...
    vrmt="none"
    mandatory="type"
    optional="name os acmt exmt user admin addr vrmt host vmhome vmdata"
    parseParameters $@
