...
# Version 0.8.0, released 03-05-2011.
#
...
known_osmts="none embedded-like unix-like windows-like aix5 aix6 arch2kx debian4 debian5 debian6 dfbsd2 esx4 esxi4 fbsd7 fbsd8 fedora13 fedora14 hpux1123 hpux1131 macosx6 maemo5 nbsd4 nbsd5 obsd4 osuse11 rhel4 rhel5 rhel6 sles10 sles11 sol10 ubuntu8 ubuntu10 win2k3 win2k8 win7 winxp"
...
toolFinder(){
	...
	echo "tools_session='awk cut grep nmap nslookup sed tr cygpath tasklist'" > $usrcfd/cfg/tools.required
    echo "tools_execute='plink psexec'"                                      >> $usrcfd/cfg/tools.required
    echo "tools_access='cryptrdp5 mstsc putty'"                              >> $usrcfd/cfg/tools.required
    ...
    echo "tools_tab='screen'"                                                >> $usrcfd/cfg/tools.required
    ...
    cat $usrcfd/cfg/tools.found | sed "s| '|'|g" > /tmp/tools.found
    mv /tmp/tools.found $usrcfd/cfg/tools.found
}
...
if [ "$privy" != 0 ]; then privy="runas /user:administrator"; else unset privy; fi
...
if [ -e "$HOME/.ssh/id_dsa.ppk" ]; then
    sshkey="$(cygpath -sm $HOME/.ssh/id_dsa.ppk)"
    sshpub="$(cygpath -sm $HOME/.ssh/id_dsa.pub)"
    sshopts="-i $sshkey"
elif [ -e "$HOME/.ssh/id_rsa.ppk" ]; then
    sshkey="$(cygpath -sm $HOME/.ssh/id_rsa.ppk)"
    sshpub="$(cygpath -sm $HOME/.ssh/id_rsa.pub)"
    sshopts="-i $sshkey"
fi
...
if [ "$sshkey" -a "$agent" = true ]; then
    ...
    if [ ! "$running" ]; then
        echo "you have a private key; loading into pageant"
        pageant "$sshkey" 2>/dev/null &
        sleep 8
    fi
fi
...
sshSendKey(){
    ...
    local sshexec="plink -x $sshopts $sshuser@$addr"
    local sshcopy="pscp $sshopts -scp -q -l $sshuser $sshpub $addr:/tmp/pubkey"
...
# addrState() - Checks the state of a given ip address with as little response
# time as possible.
addrState(){
    if [ "$debug" = true ]; then echo addrState: calling addrState with: $@ >> $logfile; fi
    local addr=$1
    
    if [ "$resilient" = true ];then
        local command="ping -n 2 -w 2000  $addr"
        local match=" from "
    else
        local command="nmap -n -T5 -PE -sP $addr"
        local match=" is up "
    fi
    
    if [ "$addr" = 127.0.0.1 -o "$addr" = localhost ]; then
        echo reachable
    else
        $privy $command 2> /dev/null | grep -q "$match"  && echo reachable || echo unreachable
    fi
}
...
portState(){
    ...
    if [ "$resilient" = true ];then
        local command="nmap -n -T1 -PN -p $port $addr"
        local match=" open "
    else
        local command="nmap -n -T5 -PN -p $port $addr"
        local match=" open "
    fi
...
returnState(){
    ...
    case $1 in
        ...
        if [ "$type" != "group" ]; then
        	echo -n "groups: "
            local groups=$($0 list group | grep "$name" | cut -d "(" -f 1)
            if [ "$groups" ]; then
        ...
        if [ "$type" != "service" ]; then    
            echo -n "services: "
            local services=$($0 list service | grep "$name" | cut -d "(" -f 1)
            if [ "$services" ]; then
        ...
        if [ "$type" = "host" ]; then    
            echo -n "guests: "
            local guests=$($0 list guest | grep "$name" | cut -d "(" -f 1)
            if [ "$guests" ]; then
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        $privy nmap -n -T5 -PE -oG $(cygpath -m /tmp/session.discover.out) -sP $range > /dev/null 2>&1 < /dev/null
        for addr in $(cat /tmp/session.discover.out | grep Up | cut -d " " -f 2 | sed -e 's/^[[:space:]]*//'); do 
            local type=host
            local name=$(nslookup $addr 2> /dev/null | grep Name | cut -d ":" -f 2 | cut -d "." -f 1 | sed -e 's/^[[:space:]]*//')
            if [ ! "$name" ]; then
...
checkState(){
    ...
    if [ "$debug" = true ]; then echo checkState: $name \($addr\): state is now $state >> $logfile; fi

    if [ "$type" = service ]; then
        if [ $port = none -o $addr = none ]; then
            svstate=$(${svmt}ServiceHandler state)        
            ...
            svstate=$(portState $addr $port)
        fi
        
        if [ $svstate = open ]; then
            state=on
        elif [ $svstate = closed ]; then
            state=$(parseEntry $host ; noneVirtHandler state)
            if [ $state = reachable ]; then
                state=$(${svmt}ServiceHandler state)
        ...
        if [ "$debug" = true ]; then echo checkState: $name \($addr\): is a service, state is now $state >> $logfile; fi
    ...
    if [ $state = reachable ]; then
        if [ $acmt = $exmt ]; then
            acstate=$(${acmt}AccessHandler state) 
            exstate=$acstate
        else
            acstate=$(${acmt}AccessHandler state) 
            exstate=$(${exmt}ExecHandler state)
        fi

        if [ $acstate = closed -o $exstate = closed ]; then
            state=busy
        else
            state=on
        fi
        
        if [ "$debug" = true ]; then echo checkState: $name \($addr\): has acmt=$acstate exmt=$exstate so state=$state >> $logfile; fi
        
    elif [ $state = unreachable -a $type = guest ]; then
    
        if [ "$debug" = true ]; then echo checkState: $name \($addr\): is a guest and state is $state. Asking $host >> $logfile; fi
    
        state=$(parseEntry $host ; noneVirtHandler state)
        if [ $state = reachable ]; then
        	
           if [ "$debug" = true ]; then echo checkState: $name \($addr\): $host is $state so continuing >> $logfile; fi
        
            state=$(${vrmt}VirtHandler state)
            if [ $state = running ]; then
            	
            	if [ "$debug" = true ]; then echo checkState: $name \($addr\): $host says $name is running >> $logfile; fi
    ...
    if [ "$debug" = true ]; then echo checkState: $name \($addr\): final state is $state >> $logfile; fi
}
...
kvmVirtHandler(){
    ...
    case "$1" in
        ...
        if [ -e "$usrcfd/tpl/$vrmt.stf" ]; then
            ...
            local result=$($0 send $host --admin --source=/tmp/session.create.$name/$name.xml --target=\"$vmdata/qemu\")
...
vmwVirtHandler(){
    ...
    case "$1" in
        ...
        if [ -e "$usrcfd/tpl/$vrmt.stf" ]; then
            ...
            local result=$($0 send $host --source=/tmp/session.create.$name --target=\"$vmdata/$name\")
...
vmfVirtHandler(){
    ...
    case "$1" in
        ...
        if [ -e "$usrcfd/tpl/$vrmt.stf" ]; then
            ...
            mkdir /tmp/session.create.$name
            cp $usrcfd/sys/$name/$name.vmx  /tmp/session.create.$name
            local result=$($0 send $host --source=/tmp/session.create.$name --target=\"$vmdata/$name.vmwarevm\")
            rm -f /tmp/session.create.$name/$name.vmx
            rmdir /tmp/session.create.$name
...
esxVirtHandler(){
    ...
    case "$1" in
        ...
        if [ -e "$usrcfd/tpl/$vrmt.stf" ]; then
            ...
            mkdir /tmp/session.create.$name
            cp $usrcfd/sys/$name/$name.vmx  /tmp/session.create.$name
            local result=$($0 send $host --admin --source=/tmp/session.create.$name --target=\"$vmdata/$name\")
            rm -f /tmp/session.create.$name/$name.vmx
            rmdir /tmp/session.create.$name
...
scriptedServiceHandler(){
    ...
    case "$1" in
        state)
        command="$svstatus"
        local result="$(${exmt}ExecHandler runasservice)"
        if [ "$result" ]; then
            echo running
        else
            echo off
        ...
        if [ $state = off ]; then
            svname=$name
            parseEntry $host
            local result=$($0 tell $svname --service --command="$svstart")
            name=$svname
            state=starting
        else
        ...
        if [ $state = on ]; then
            svname=$name
            parseEntry $host
            local result=$($0 tell $svname --service --command="$svstop")
            name=$svname
            state=halting
        else
...
httpAccessHandler(){
    ...
    if [ "$port" -a "$port" != none ]; then
        local acstate=$(portState $addr $port)
        local proto=http
    elif [ "$(portState $addr 443)" = open ]; then
        local acstate=open
        local port=443
    ...
    case "$1" in
        ...
        ;;
        access)
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            explorer $proto://$addr:$port &
        fi
        ;;
        *)
    esac
...
rdpAccessHandler(){
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            mkdir -p $usrcfd/sys/$name
            local rdpfile=$usrcfd/sys/$name/$name.rdp
            ...
            if [ $xsastype = user -a ! -z "$upwd" ]; then
                local rdphash=$(cryptrdp5 $upwd)
                echo password 51:b:$rdphash >> $rdpfile
            elif [ $xsastype = admin -a ! -z "$apwd" ]; then
                local rdphash=$(cryptrdp5 $apwd)
                echo password 51:b:$rdphash >> $rdpfile
            fi
            chmod 600 $rdpfile

            mstsc $(cygpath -w $rdpfile) 2>/dev/null &
        fi
        ;;
        *)
    esac
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        elif [ "$exstate" = open ]; then
            echo "@echo off"                                                                    > /tmp/session.command.$name.bat
            echo "psexec \\\\$addr -h -u $user $upwdopts cmd.exe /c \"$command\" 2> nul"       >> /tmp/session.command.$name.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$name.bat)
            rm /tmp/session.command.$name.bat
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        fi
        ;;
        runasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "@echo off"                                                                    > /tmp/session.command.$name.bat
            echo "psexec \\\\$addr -h -u $admin $apwdopts cmd.exe /c \"$command\" 2> nul"      >> /tmp/session.command.$name.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$name.bat)
            rm /tmp/session.command.$name.bat
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        ...
        runasservice)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "changing of local privileges using psexec not implemented yet"
        elif [ "$type" = service ]; then
            svname=$name
            svuser=$user
            svupwdopts=$upwdopts
            parseEntry $host
            echo "@echo off"                                                                  > /tmp/session.command.$svname.bat
            echo "psexec \\\\$addr -h -u $svuser $svupwdopts cmd.exe /c \"$command\" 2> nul" >> /tmp/session.command.$svname.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$svname.bat)
            rm /tmp/session.command.$svname.bat
        else
            echo "can't run a command as a service when talking to a $type"
        fi
        ;;
        sendasuser)
        if [ "$addr" = "127.0.0.1" ]; then
            local source=\"$(cygpath -w $source)\"
            local target="$target"
            echo robocopy /e $source $target > /tmp/session.send.$name.bat
            cmd.exe /c $(cygpath -m /tmp/session.send.$name.bat)
            rm /tmp/session.send.$name.bat
        elif [ "$exstate" = open ]; then
            local share=$(echo $target | cut -d: -f1 | sed 's|"||g')
            local source=\"$(cygpath -w $source)\"
            local target=$(echo $target | sed "s|$share:|\\\\\\\\$addr\\\\\\$share\\$|g" | sed "s|/|\\\\|g")
        ...
        sendasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "sending local files with elevated privileges when using robocopy not implemented yet"
        elif [ "$exstate" = open ]; then
            local share=$(echo $target | cut -d: -f1 | sed 's|"||g')
            local source=\"$(cygpath -w $source)\"
            local target=$(echo $target | sed "s|$share:|\\\\\\\\$addr\\\\\\$share\\$|g" | sed "s|/|\\\\|g")
        ...
        sendasservice)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "sending local files with changed privileges when using robocopy not implemented yet"
        elif [ "$type" = service ]; then
            svname=$name
            svuser=$user
            svupwd=$upwd
            ...
            local source=\"$(cygpath -w $source)\"
            local target=$(echo $target | sed "s|$share:|\\\\\\\\$addr\\\\\\$share\\$|g" | sed "s|/|\\\\|g")
            net use \\\\$addr\\$share\$ /user:"$svuser" "$svupwd" > /dev/null 2>&1 < /dev/null
            echo robocopy $source $target > /tmp/session.send.$svname.bat
            cmd.exe /c $(cygpath -m /tmp/session.send.$svname.bat)
            rm /tmp/session.send.$svname.bat
            net use \\\\$addr\\$share\$ /delete
        else
            echo "can't send data as a service when talking to a $type"
        fi
        ;;
        *)
    esac
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "@echo off"                                        > /tmp/session.command.$name.bat
            echo "plink -x $sshopts $user@$addr \"$command\""       >> /tmp/session.command.$name.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$name.bat)
            rm /tmp/session.command.$name.bat
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        ...
        runasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "elevation of local privileges when using plink not implemented yet"
        elif [ "$exstate" = open ]; then
            echo "@echo off"                                        > /tmp/session.command.$name.bat
            echo "plink -x $sshopts $admin@$addr \"$command\""      >> /tmp/session.command.$name.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$name.bat)
            rm /tmp/session.command.$name.bat
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        ...
        runasservice)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "changing of local privileges when using plink not implemented yet"
        elif [ "$type" = service ]; then
            svname=$name
            svuser=$user
            parseEntry $host
            echo "@echo off"                                        > /tmp/session.command.$svname.bat
            echo "plink -x $sshopts $svuser@$addr \"$command\""     >> /tmp/session.command.$svname.bat
            cmd.exe /c $(cygpath -m /tmp/session.command.$svname.bat)
            rm /tmp/session.command.$svname.bat
        else
            echo "can't run a command as a service when talking to a $type"
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            pscp $sshopts -scp -p -q -r "$(cygpath -m $source)" $user@$addr:"$target"
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        fi
        ;;
        sendasadmin)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "sending local files with elevated privileges when using pscp not implemented yet"
        elif [ "$exstate" = open ]; then
            pscp $sshopts -scp -p -q -r "$(cygpath -m $source)" $admin@$addr:"$target"
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        fi
        ;;
        sendasservice)
        if [ "$addr" = "127.0.0.1" ]; then
            echo "sending local files with changed privileges when using pscp not implemented yet"
        elif [ "$type" = service ]; then
            svuser=$user
            parseEntry $host
            pscp $sshopts -scp -p -q -r "$(cygpath -m $source)" $svuser@$addr:"$target"
        else
            echo "can't send data as a service when talking to a $type"
        fi
        ;;
...
case "$1" in 
    ...
    if [ "$type" = "host" -a "$vrmt" = "none" ]; then
        ...
        mandatories="type svmt acmt exmt user addr port host"
        parseParameters $@
        addConf "$type $name($svmt,$acmt,$exmt,$user,$addr,$port,$host)"
        
    elif [ "$type" = "service" -a "$svmt" != "none" ]; then
        mandatories="type svmt acmt user addr port host svstatus svstart svstop"
        parseParameters $@
        addConf "$type $name($svmt,$acmt,$exmt,$user,$addr,$port,$host)"
    ...
    ;;
    recheck)
    rm -f "$usrcfd/cfg/session/tools.required" "$usrcfd/cfg/tools.found"
    toolFinder
    ;;
    list)
    case $2 in
        ...
        ;;
        service|services)
        grep  '^service' "$config" | sed 's|^service ||g'
        ;;
        all)
        ...
        *)
        if [ "$2" ]; then
            ...
            cat "$config" | grep -v "^#" | grep "service $2("
    ...
    *)
    ...
    echo "state      - checks the state of a host, guest, service or group."
    ...
    echo "recheck    - reinitializes session's required and detected tools."
    ...
    echo "--mode     - serial (default), stateful or parallel (not implemented yet)."
    echo ""    
