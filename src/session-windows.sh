...
# Version 0.8.3, released 10-05-2011.
#
...
sshSendKey(){
    ...
    local sshexec="plink -x $sshopts -l \"$sshuser\" $addr"
...
portState(){
    ...
    if [ "$resilient" = true ];then
        ...
        if [ "$scantype" = connect ];then
            ...
            local command="nmap -n -T5 --host-timeout $timeout -PN -sS -p $port $addr"
            local match=" open "
        fi
    fi
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local escaped=$(echo "$command" | sed 's|"|\\"|g')
            echo "@echo off"                                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "plink -x $sshopts -l \"$user\" $addr \"$escaped\""                                            >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local escaped=$(echo "$command" | sed 's|"|\\"|g')
            echo "@echo off"                                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "plink -x $sshopts -l \"$admin\" $addr \"$escaped\""                                           >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
            	local escaped=$(echo "$command" | sed 's|"|\\"|g')
                echo "@echo off"                                                                                 > "$usrcfd/tmp/session.tell.$svname.bat"
                echo "plink -x $sshopts -l \"$svuser\" $addr \"$escaped\""                                      >> "$usrcfd/tmp/session.tell.$svname.bat"
                cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.tell.$svname.bat")
                rm "$usrcfd/tmp/session.tell.$svname.bat"
            fi
        else
