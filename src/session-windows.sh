...
# Version 0.8.1, released 06-05-2011.
#
...
delConf() {
    ...
    cat "$config" | sed -e "/$check/d" > "$usrcfd/tmp/session.conf.tmp"
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        $privy nmap -n -T5 -PE -oG $(cygpath -sm "$usrcfd/tmp/session.discover.out") -sP $range > /dev/null 2>&1 < /dev/null
...
rdpAccessHandler(){
    ...
    case "$1" in
        ...
        if [ "$acstate" = active -a "$addr" != 127.0.0.1 ]; then
            ...
            mstsc $(cygpath -sm "$rdpfile") 2>/dev/null &
        else
            echo "$1 failed: type=$type addr=$addr xsastype=$xsastype xsasuser=$xsasuser acmt=$acmt acstate=$acstate"
        fi
        ;;
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        elif [ "$exstate" = active -a "$type" != service ]; then
            echo "@echo off"                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "psexec \\\\$addr -h -u $user $upwdopts cmd.exe /c \"$command\" 2> nul"        >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "@echo off"                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "psexec \\\\$addr -h -u $admin $apwdopts cmd.exe /c \"$command\" 2> nul"       >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                echo "@echo off"                                                                  > "$usrcfd/tmp/session.tell.$svname.bat"
                echo "psexec \\\\$addr -h -u $svuser $svupwdopts cmd.exe /c \"$command\" 2> nul" >> "$usrcfd/tmp/session.tell.$svname.bat"
                cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$svname.bat")
                rm "$usrcfd/tmp/session.tell.$svname.bat"
            fi
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
        ...
        sendasuser)
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            local source=\"$(cygpath -sm $source)\"
            local target="$target"
            echo robocopy /e $source $target                                                     > "$usrcfd/tmp/session.send.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.send.$name.bat")
            rm "$usrcfd/tmp/session.send.$name.bat"
        elif [ "$exstate" = active -a "$type" != service ]; then
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local target=$(echo $target | sed "s|$share:|\\\\\\\\$addr\\\\\\$share\\$|g" | sed "s|/|\\\\|g")
            net use \\\\$addr\\$share\$ /user:"$admin" "$apwd" > /dev/null 2>&1 < /dev/null
            echo robocopy $source $target                                                        > "$usrcfd/tmp/session.send.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.send.$name.bat")
            rm "$usrcfd/tmp/session.send.$name.bat"
            net use \\\\$addr\\$share\$ /delete
        else
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                ...
                echo robocopy $source $target                                                    > "$usrcfd/tmp/session.send.$svname.bat"
                cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.send.$svname.bat")
                rm "$usrcfd/tmp/session.send.$svname.bat"
                net use \\\\$addr\\$share\$ /delete
            fi
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "@echo off"                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "plink -x $sshopts $user@$addr \"$command\""                                   >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "@echo off"                                                                     > "$usrcfd/tmp/session.tell.$name.bat"
            echo "plink -x $sshopts $admin@$addr \"$command\""                                  >> "$usrcfd/tmp/session.tell.$name.bat"
            cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$name.bat")
            rm "$usrcfd/tmp/session.tell.$name.bat"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                echo "@echo off"                                                                 > "$usrcfd/tmp/session.tell.$svname.bat"
                echo "plink -x $sshopts $svuser@$addr \"$command\""                             >> "$usrcfd/tmp/session.tell.$svname.bat"
                cmd.exe /c $(cygpath -sm "$usrcfd/tmp/session.tell.$svname.bat")
                rm "$usrcfd/tmp/session.tell.$svname.bat"
            fi
        else
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            pscp $sshopts -scp -p -q -r -l "$user" "$(cygpath -sm $source)" $addr:"$target"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            pscp $sshopts -scp -p -q -r -l "$admin" "$(cygpath -sm $source)" $addr:"$target"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            sshExecHandler state
            if [ "$exstate" = active ]; then
                pscp $sshopts -scp -p -q -r -l "$svuser" "$(cygpath -sm $source)" $addr:"$target"
            fi
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
...
case "$1" in 
    ...
    ;;
    reinit)
    rm -f "$usrcfd/cfg/options.conf" "$usrcfd/cfg/tools.required" "$usrcfd/cfg/tools.found"
    $0 version
    ;;
    list)
    case $2 in
