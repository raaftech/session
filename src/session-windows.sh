...
smbExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local target="$target"
            echo "@echo off"                                                                                     > "$usrcfd/tmp/session.send.$name.bat"
            echo "robocopy /e $source $target >nul 2>&1"                                                        >> "$usrcfd/tmp/session.send.$name.bat"
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.send.$name.bat")
            rm "$usrcfd/tmp/session.send.$name.bat"
        elif [ "$exstate" = active -a "$type" != service ]; then
            local share=$(echo "`cygpath -m "$target"`" | cut -d: -f1 | sed 's|"||g')
            local strip="$(echo $source | sed 's|"||g')"
            local source=\"$(cygpath -m $strip)\"
            local target=\"$(echo "`cygpath -m "$target"`" | sed "s|$share:|\\\\\\\\$addr\\\\\\$share\\$|g" | sed "s|/|\\\\|g")\"
            net use \\\\$addr\\$share\$ /user:"$admin" "$apwd" > /dev/null 2>&1 < /dev/null
            echo "@echo off"                                                                                     > "$usrcfd/tmp/session.send.$name.bat"
            echo "robocopy /e $source $target >nul 2>&1"                                                        >> "$usrcfd/tmp/session.send.$name.bat"
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.send.$name.bat")
            rm "$usrcfd/tmp/session.send.$name.bat"
            net use \\\\$addr\\$share\$ /delete > /dev/null 2>&1 < /dev/null
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
