...
# Version 0.7.5, released 04-01-2011.
#
...
# nmap or when running command-asadmin on localhost.
privy=0
if [ $privy = 1 ]; then
    privy="runas"
else
...
tools_extras="cryptrdp5 nmap putty plink pscp psexec"
...
noneVirtHandler(){
    ...
    case "$1" in
        state)
        if [ "$privy" ]; then
            # Don't use runas ($privy) since windows asks about privilege escalation automatically.
