...
# Version 0.8.2, released 07-05-2011.
#
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        $privy nmap -n -T5 -PE -oG $(cygpath -m "$usrcfd/tmp/session.discover.out") -sP $range > /dev/null 2>&1 < /dev/null
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.tell.$name.bat")
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                ...
                cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.tell.$svname.bat")
        ...
        sendasuser)
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            local strip="$(echo $source | sed 's|"||g')"
            local source=\"$(cygpath -m $strip)\"
            local target="$target"
            echo robocopy /e $source $target                                                     > "$usrcfd/tmp/session.send.$name.bat"
            cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.send.$name.bat")
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                ...
                cmd.exe /c $(cygpath -m "$usrcfd/tmp/session.send.$svname.bat")
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local strip="$(echo $source | sed 's|"||g')"
            pscp $sshopts -scp -p -q -r -l "$user" "$(cygpath -m $strip)" $addr:"$target"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            local strip="$(echo $source | sed 's|"||g')"
            pscp $sshopts -scp -p -q -r -l "$admin" "$(cygpath -m $strip)" $addr:"$target"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                local strip="$(echo $source | sed 's|"||g')"
                pscp $sshopts -scp -p -q -r -l "$svuser" "$(cygpath -m $strip)" $addr:"$target"
            fi
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        fi
