...
toolFinder(){
    ...
    echo "tools_access='open osascript'"                                          >> $usrcfd/cfg/tools.required
    ...
    for tooltype in $tooltypes; do
        ...
        for tool in $values; do
            if [ "$(basename "`which $tool 2> /dev/null`")" ]; then
                if [ $color = true ]; then 
                    echo -n "`tput setaf 2`$tool`tput sgr0` "
                else
                    echo -n "$tool "
                fi
                ...
                if [ $color = true ]; then 
                    echo -n "`tput setaf 1`$tool`tput sgr0` "
                else
                    echo -n "($tool) "
                fi
            fi
...
addrState(){
    ...
    if [ "$resilient" = true -o ! "$privy" ];then
        local command="ping -c 2 -W 2000  $addr"
        local match=" from "
    else
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        for addr in $(cat /tmp/session.discover.out | grep Up | cut -d " " -f 2 | sed -e 's/^[[:space:]]*//'); do 
            local type=host
            local name=$(nslookup $addr 2> /dev/null | grep -w name | cut -d "=" -f 2 | cut -d "." -f 1 | sed -e 's/^[[:space:]]*//')
            if [ ! "$name" ]; then
...
sshAccessHandler(){
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            echo "tell application \"Terminal\"" >> /tmp/session.access.$name.scpt
            echo "tell application \"System Events\" to tell process \"Terminal\" to keystroke \"t\" using command down" >> /tmp/session.access.$name.scpt
            echo "do script \"ssh $sshopts -Y $xsasuser@$addr\" in last tab of front window" >> /tmp/session.access.$name.scpt
            echo "end tell" >> /tmp/session.access.$name.scpt
            osascript /tmp/session.access.$name.scpt
            rm /tmp/session.access.$name.scpt
        fi
