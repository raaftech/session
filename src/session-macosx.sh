...
toolFinder(){
    ...
    echo "tools_session='awk cut grep nmap host sed tr ps'"                        > "$usrcfd/cfg/tools.required"
...
if [ "$sshkey" -a "$agent" = true ]; then
    sshagent="$usrcfd/tmp/session.ssh-agent.out"
    if [ ! -e "$sshagent" ]; then
        echo "you have a private key; loading into ssh-agent"
        ssh-agent | grep -v "^echo " > "$sshagent"
        chmod 600 "$sshagent"
        source "$sshagent"
        ssh-add
    else
...
sshAccessHandler(){
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            echo "tell application \"Terminal\"" >> "$usrcfd/tmp/session.access.$name.scpt"
            echo "tell application \"System Events\" to tell process \"Terminal\" to keystroke \"t\" using command down" >> "$usrcfd/tmp/session.access.$name.scpt"
            echo "do script \"ssh $sshopts -Y $xsasuser@$addr\" in last tab of front window" >> "$usrcfd/tmp/session.access.$name.scpt"
            echo "end tell" >> "$usrcfd/tmp/session.access.$name.scpt"
            osascript "$usrcfd/tmp/session.access.$name.scpt"
            rm "$usrcfd/tmp/session.access.$name.scpt"
        else
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "#!/bin/sh"                                                                     > "$usrcfd/tmp/session.tell.$name.sh"
            echo "winexe -U '$user%$upwd' //$addr '$command' 2> /dev/null"                      >> "$usrcfd/tmp/session.tell.$name.sh"
            sh "$usrcfd/tmp/session.tell.$name.sh"
            rm "$usrcfd/tmp/session.tell.$name.sh"
        else
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "#!/bin/sh"                                                                     > "$usrcfd/tmp/session.tell.$name.sh"
            echo "winexe -U '$admin%$apwd' //$addr '$command' 2> /dev/null"                     >> "$usrcfd/tmp/session.tell.$name.sh"
            sh "$usrcfd/tmp/session.tell.$name.sh"
            rm "$usrcfd/tmp/session.tell.$name.sh"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                ...
                echo "winexe -U '$svuser%$svupwd' //$addr '$command' 2> /dev/null"                 >> "$usrcfd/tmp/session.tell.$svname.sh"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                ...
                sh "$usrcfd/tmp/session.send.$svname.sh"
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "#!/bin/sh"                                                                     > "$usrcfd/tmp/session.tell.$name.sh"
            echo "ssh $sshopts $user@$addr '$command'"                                          >> "$usrcfd/tmp/session.tell.$name.sh"
            sh "$usrcfd/tmp/session.tell.$name.sh"
            rm "$usrcfd/tmp/session.tell.$name.sh"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" != service ]; then
            ...
            echo "#!/bin/sh"                                                                     > "$usrcfd/tmp/session.tell.$name.sh"
            echo "ssh $sshopts $admin@$addr '$command'"                                         >> "$usrcfd/tmp/session.tell.$name.sh"
            sh "$usrcfd/tmp/session.tell.$name.sh"
            rm "$usrcfd/tmp/session.tell.$name.sh"
        else
            echo "$1 failed: type=$type addr=$addr user=$user exmt=$exmt exstate=$exstate"
        ...
        if [ "$addr" = "127.0.0.1" -a "$type" = service ]; then
            ...
            if [ "$exstate" = active ]; then
                echo "#!/bin/sh"                                                                 > "$usrcfd/tmp/session.tell.$svname.sh"
                echo "ssh $sshopts $svuser@$addr '$command'"                                    >> "$usrcfd/tmp/session.tell.$svname.sh"
                sh "$usrcfd/tmp/session.tell.$svname.sh"
                rm "$usrcfd/tmp/session.tell.$svname.sh"
            fi
        else
