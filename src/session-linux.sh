...
tools_native="awk cp cut gnome-open gnome-terminal grep nslookup pgrep ping sed ssh ssh-agent ssh-add scp smbclient uuidgen"
tools_extras="nmap rdesktop winexe wmctrl xdotool"
...
if [ "$pkfile" -a "$agent" = true ]; then
    running=`ps ax | grep -i ssh-agent`
    if [ ! "$running" ]; then
...
discoveryHelper(){
    ...
    if [ "$norange" ]; then
        ...
        for addr in `cat /tmp/session.discover.out | grep Up | cut -d " " -f 2 | sed -e 's/^[ \t]*//'`; do 
            type=host
            name=$(nslookup $addr 2> /dev/null | grep -w name | cut -d "=" -f 2 | cut -d "." -f 1 | sed -e 's/^[ \t]*//')
            if [ ! "$name" ]; then
    ...
    case "$1" in
        ...
        if [ "$access" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            if [ "$?" == "0" ]; then
                ...
                xdotool key ctrl+shift+t
                sleep 0.2
                xdotool type "ssh $pkopts -Y $user@$addr"
                xdotool key Return
                xdotool key alt+t
                xdotool key Return
                xdotool type "`capsFirst $name`"
                ...
                gnome-terminal --command="ssh $pkopts -Y $user@$addr" &
            fi
        fi
        ;;
        *)
