...
# Version 0.6.5, released 15-12-2010.
#
...
tools="awk cut sed grep pgrep xdotool sleep wmctrl gnome-terminal nmap ssh scp rdesktop winexe smbclient uuidgen"
...
esxVirtHandler(){
    ...
    case "$1" in
        ...
        if [ -e "$usrcfd/tpl/$vrmt.stf" ]; then
            ...
            rm -f /tmp/session.create.$name/$name.vmx
            rmdir /tmp/session.create.$name
            result=$($0 command-asadmin $host /sbin/vmkfstools -c ${disksize}G \"$vmdata/$name/$name.vmdk\" 2>/dev/null | sed '/^$/d')
