...
toolFinder(){
	...
	echo "tools_session='awk cut grep nmap nslookup sed tr ping ps'"			   > $usrcfd/cfg/tools.required
    echo "tools_execute='ssh winexe'"                                      		  >> $usrcfd/cfg/tools.required
    echo "tools_access='gnome-open gnome-terminal pgrep rdesktop wmctrl xdotool'" >> $usrcfd/cfg/tools.required
...
addrState(){
    ...
    if [ "$resilient" = true -o ! "$privy" ];then
        local command="ping -c 2 -W 2  $addr"
        local match=" from "
    else
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            if [ "$?" == "0" ]; then
                ...
                gnome-terminal --command="telnet $addr" &
            fi
        fi
        ;;
        *)
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            if [ "$?" == "0" ]; then
                ...
                xdotool key ctrl+shift+t
                sleep 0.2
                xdotool type "ssh $sshopts -Y $xsasuser@$addr"
                xdotool key Return
                xdotool key alt+t
                xdotool key Return
                xdotool type "$(capsFirst $name)"
                ...
                gnome-terminal --command="ssh $sshopts -Y $xsasuser@$addr" &
            fi
        fi
        ;;
        *)
...
rdpAccessHandler(){
    ...
    case "$1" in
        ...
        if [ "$acstate" = open -a "$addr" != 127.0.0.1 ]; then
            ...
            echo "$upwd" | rdesktop -0 -g 1024x768 -b -B -T "$(capsFirst $name)" -u $user -p - -N -a 16 -z -x l -r disk:home=$HOME $addr 2>/dev/null &
        fi
        ;;
        *)
    esac
...
smbExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            svupwd=$upwd
            parseEntry $host
            echo "#!/bin/sh"                                               > /tmp/session.command.$svname.sh
            echo "winexe -U 'HOME/$svuser%$svupwd' //$addr '$command'"    >> /tmp/session.command.$svname.sh
            sh /tmp/session.command.$svname.sh
            rm /tmp/session.command.$svname.sh
        else
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            local target="$(echo $target | sed 's|/|\\\\|g' | cut -d: -f2 | sed 's|"||g')"
            smbcommand="mkdir \"$target\";cd \"$target\";lcd \"$source\";prompt off;recurse on;mput *;quit"
            echo "#!/bin/sh"                                                     > /tmp/session.send.$svname.sh
            echo "smbclient //$addr/$share -U \"$svuser%$svupwd\" -c '$smbcommand'" >> /tmp/session.send.$svname.sh
            sh /tmp/session.send.$svname.sh
            rm /tmp/session.send.$svname.sh
        else
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "#!/bin/sh"                                        > /tmp/session.command.$name.sh
            echo "ssh $sshopts $user@$addr '$command'"              >> /tmp/session.command.$name.sh
            sh /tmp/session.command.$name.sh
            rm /tmp/session.command.$name.sh
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            echo "#!/bin/sh"                                        > /tmp/session.command.$name.sh
            echo "ssh $sshopts $admin@$addr '$command'"             >> /tmp/session.command.$name.sh
            sh /tmp/session.command.$name.sh
            rm /tmp/session.command.$name.sh
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            parseEntry $host
            echo "#!/bin/sh"                                        > /tmp/session.command.$svname.sh
            echo "ssh $sshopts $svuser@$addr '$command'"            >> /tmp/session.command.$svname.sh
            sh /tmp/session.command.$svname.sh
            rm /tmp/session.command.$svname.sh
        else
            echo "can't run a command as a service when talking to a $type"
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            scp $sshopts -r "$source" $user@$addr:"$target"
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        fi
        ;;
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            scp $sshopts -r "$source" $admin@$addr:"$target"
        else
            echo "can't reach $name on $addr using $exmt because state is $exstate"
        fi
        ;;
        ...
        if [ "$addr" = "127.0.0.1" ]; then
            ...
            svuser=$user
            parseEntry $host
            scp $sshopts -r "$source" $svuser@$addr:"$target"
        else
            echo "can't send data as a service when talking to a $type"
        fi
        ;;
