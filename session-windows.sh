...
tools="awk cut sed grep portqry putty plink pscp mstsc cryptrdp5 psexec"
...
if [ -e $HOME/.ssh/id_dsa.pkk ]; then
    pkkopts="-i `cygpath -w $HOME/.ssh/id_dsa.pkk`"
elif [ -e $HOME/.ssh/id_rsa.pkk ]; then
    pkkopts="-i `cygpath -w $HOME/.ssh/id_rsa.pkk`"
fi;
...
portState(){
    ...
    echo 'portqry -n $addr -e $port 2> /dev/null | grep LISTENING'                 >> /tmp/portstate.$name
...
noneVirtHandler(){
    ...
    case "$1" in
        state)
        echo "ping -w 2000 -n 2 $addr" > /tmp/hoststate
...
sshSendKey(){
    ...
    if [ ! -z $sshkey ]; then
        ...
        pscp $pkkopts -q "`cygpath -w $sshkey`" $sender@$addr:/tmp/pubkey
        if [ "$?" == "0" ]; then
...
# system using the psexec protocol and the psexec program.
pseExecHandler(){
    ...
    case "$1" in
        ...
        if [ $execute = open ]; then
            psexec \\\\$addr -h -u $user $upwdopts $command
        fi;
        ;;
        runasadmin)
        if [ $execute = open ]; then
            psexec \\\\$addr -h -u $admin $apwdopts $command
        fi;
        ;;
        sendasuser)
        if [ $execute = open ]; then
            echo "Not implemented yet on session for windows."
...
sshExecHandler(){
    ...
    case "$1" in
        ...
        runasuser)
        if [ $execute = open ]; then
            plink $pkkopts $user@$addr "$command"
        fi;
        ;;
        runasadmin)
        if [ $execute = open ]; then
            plink $pkkopts $admin@$addr "$command"
        fi;
        ;;
        sendasuser)
        if [ $execute = open ]; then
            pscp $pkkopts -pqr "`cygpath -w $source`" $user@$addr:$target
        fi;
        ;;
        sendasadmin)
        if [ $execute = open ]; then
            pscp $pkkopts -pqr "`cygpath -w $source`" $admin@$addr:$target
        fi;
        ;;
        *)
    esac
